...
2026年7月12日に架空データのみを使って実機テストを行い、7項目中6項目が合格しました。完成条件の「5種類以上」を満たしています。
...
短時間送信では2件を1秒差で送信し、重複しない問い合わせ番号が発行され、各回答が1回ずつ処理されました。二重送信防止テストでは、同じ回答IDの再処理時に管理行とメール処理が増えないことを確認しました。
